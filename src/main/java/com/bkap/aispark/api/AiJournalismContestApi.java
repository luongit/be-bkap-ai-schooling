...
public class AiJournalismContestApi {
	...
	public Map<String, Object> gradeByOpenAi(@PathVariable Long id, HttpServletRequest request) {
		try {
			...
			ChatCompletionRequest chatReq = ChatCompletionRequest.builder().model("gpt-4o-mini")
					.messages(List.of(new ChatMessage("system",
							"Bạn là giám khảo cuộc thi 'AI Nhà Báo Nhí'. Hãy chấm bài viết theo tiêu chí và trả về kết quả JSON."),
							new ChatMessage("user", prompt)))
					.temperature(0.2).stream(false).build();
			...
			// 🔹 Trừ credit
			int totalTokens = Optional.ofNullable(result.getUsage()).map(u -> u.getTotalTokens()).map(Long::intValue)
					.orElse(0);
			creditService.deductByTokenUsage(userId, "AI_CONTEST", totalTokens, "entry-" + id);

			// 🔹 Trả kết quả
			return Map.of("status", "success", "entryId", id, "score", aiScore, "feedback", feedback, "remainingCredit",
					creditService.getRemainingCredit(userId));
	...
	private String buildGradingPrompt(AiJournalismEntry entry, List<AiJournalismRubric> rubrics) {
		...
		for (AiJournalismRubric r : rubrics) {
			rubricText.append("- ").append(r.getCriterion()).append(": ").append(r.getDescription())
					.append(" (trọng số: ").append(r.getWeight()).append(")\n");
		}

		return String.join("\n", "Đây là một bài viết của học sinh tham gia cuộc thi 'AI Nhà Báo Nhí'.",
				"Hãy đọc kỹ và chấm điểm theo tiêu chí dưới đây:", rubricText.toString(), "", "Bài viết:",
				"Tiêu đề: " + entry.getTitle(), "Nội dung: \n" + entry.getArticle(), "",
